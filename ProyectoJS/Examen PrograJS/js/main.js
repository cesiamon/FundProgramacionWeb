...
function verificarSede(sedeId) {
  ...
  if (sede === 'Escazu') {
    insertarInformacion(
      'CLINICA ESCAZU',
      '+506 6978-9344',
      'escazu@dentalcareinfo.com'
    )
  } else if (sede === 'San Pedro') {
    insertarInformacion(
      'CLINICA SAN PEDRO',
      '+506 6975-8848',
      'sanpedro@dentalcareinfo.com'
    )
  } else if (sede === 'Heredia') {
    insertarInformacion(
      'CLINICA HEREDIA',
      '+506 6976-8846',
      'heredia@dentalcareinfo.com'
    )
  }
}
...
function agregarProducto(nombre, precio) {
  ...
  for (var i = 0; i < listaProductos.children.length; i++) {
    ...
    var precioItem = parseFloat(
      listItem.innerText.split(' - $')[1].replace(',', '').trim()
    )
  ...
  descuentoSeguroPorcentaje = parseFloat(
    document.getElementById('descuentoSeguro').innerText
  )
  descuentoPacientePorcentaje = parseFloat(
    document.getElementById('descuentoPaciente').innerText
  )
  ...
  totalConDescuentos =
    totalPrecio - descuentoSeguroMonto - descuentoPacienteMonto
...
function validateForm2() {
  ...
  if (EmptyRatio2 === true) {
    ...
    let label5 = document.querySelector(
      `label[for='${comunicacionPersonal.id}']`
    )
  ...
  if (EmptySpaceCheckBox2 === true) {
    ...
    let label2 = document.querySelector(
      `label[for='${serviciosAdicionales.id}']`
    )
...
function calcularEncuesta() {
  ...
  var experience1Value = document.querySelector(
    'input[name="experience1"]:checked'
  ).value
  var experience2Value = document.querySelector(
    'input[name="experience2"]:checked'
  ).value
  var experience3Value = document.querySelector(
    'input[name="experience3"]:checked'
  ).value

  // Count the number of "veryPositive" responses
  var veryPositiveCount = [
    experience1Value,
    experience2Value,
    experience3Value,
  ].filter((value) => value === 'veryPositive').length
  var positiveCount = [
    experience1Value,
    experience2Value,
    experience3Value,
  ].filter((value) => value === 'positive').length
  var neutralCount = [
    experience1Value,
    experience2Value,
    experience3Value,
  ].filter((value) => value === 'neutral').length
  var negativeCount = [
    experience1Value,
    experience2Value,
    experience3Value,
  ].filter((value) => value === 'negative').length
  var veryNegativeCount = [
    experience1Value,
    experience2Value,
    experience3Value,
  ].filter((value) => value === 'veryNegative').length

  // Determine the overall result based on the conditions
  if (veryPositiveCount === 3) {
    document.getElementById('res').innerText = 'Excelente! Muchas gracias por sus comentarios.(Encuesta Excelente)'
  } else if (
    (veryPositiveCount === 2 && positiveCount === 1) ||
    (veryPositiveCount === 2 && neutralCount === 1) ||
    (veryPositiveCount === 2 && veryNegativeCount === 1) ||
    positiveCount === 3 ||
    (positiveCount === 2 && veryPositiveCount === 1) ||
    (positiveCount === 2 && neutralCount === 1)
  ) {
    document.getElementById('res').innerText = 'Muchas gracias :) (Encuesta Positiva)'
  } else if (
    neutralCount === 3 ||
    (neutralCount === 2 && positiveCount === 1)
  ) {
    document.getElementById('res').innerText = 'Encuesta Enviada! (Encuesta Neutral) '
  } else if (neutralCount === 2 && negativeCount === 1) {
    document.getElementById('res').innerText = 'Lamentamos su experiencia, mejoraremos. (Encuesta negativa)'
  } else if (
    (negativeCount === 2 && neutralCount === 1) ||
    (veryNegativeCount === 2 && negativeCount === 1)
  ) {
    document.getElementById('res').innerText = 'Lamentamos su terrible experiencia. (Encuesta muy negativa)'
  }
}